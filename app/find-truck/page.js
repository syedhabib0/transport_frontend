  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        if (driverCode === 200) {
          const statusData = Object.keys(driverStatus.data).map((key) => ({
            value: key,
            label: driverStatus.data[key],
          }));
            ...
            <Row className="content space-x-5">
              {!active ? (
                <Col
                  md={12}
                  lg={5}
                  className="md:mr-2 flex flex-col pb-40 space-y-5 mt-4 p-4 bg-white border-gradient border-gradient-color"
                >
