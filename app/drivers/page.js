...
import { Button, Col, Container, Dropdown, Form, FormGroup, Pagination, Row, Table } from "react-bootstrap";
                  ...
                  {drivers && drivers.length > 0 ? (
                    drivers.map((driver, key) => (
                      <tr className="align-middle" key={key}>
                        ...
                        <td>{driver.email}</td>
                        <td>{driver.profile ? driver.profile.phone : "Not Available"}</td>
                        <td>
                          <Button
                            className={`p-1 text-xs w-24 border-none rounded-pill text-capitalize ${
                              driver.driver ? getStatusColor(driver.driver.status) : "bg-red-500"
                            }`}
                            onClick={() => handleShowModal(driver.driver.id, driver.driver.status)}
                          >
                            {driver.driver && driver.driver.status ? driver.driver.status : "Not Available"}
                          </Button>
                        </td>
                        <td>
                          <Dropdown>
