...
const EditLoad = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      console.log(body);
      try {
        const { data, status } = await axios.post(`${apis.loads}/update-load/${id}`, body, {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        const { data: formData, status: fromDataStatus } = await axios.get(`${apis.loads}/${id}`, {
          headers: { Authorization: `Bearer ${access_token}` },
        });
        if (fromDataStatus === 200) {
          const {
            data: {
              bill_id,
              delivery_date,
