...
const NewLoadModal = ({ show, handleClose }) => {
    ...
    useEffect(() => {
        const fetchDrivers = async () => {
            try {
                // Make an API call to your Laravel endpoint to fetch driver data
                const response = await axios.get(apis.drivers) // Adjust the endpoint URL

                // Extract the driver data from the response
                const drivers = response.data
                // console.log("34: ", drivers.drivers)

                // Map the driver data to the format expected by react-select
                const driverOptionsData = drivers.drivers.map(driver => ({
                    value: driver.driver ? driver.driver.id : driver.id,
                    label: `${driver.first_name} ${driver.last_name}`,
                }))

                setDriverOptions(driverOptionsData)
                // console.log("43: ",driverOptions)
            } catch (error) {
                console.error('Error fetching driver data:', error)
            }
        }

        // Call the fetchDrivers function to load driver data
        fetchDrivers()
    }, [])
                ...
                <Form onSubmit={formik.handleSubmit}>
                    <FormGroup className="mb-3">
                        ...
                        <InputGroup className="mb-3">
                            ...
                            /> */}
                            {/* {driverOptions.map((currentValue, index) => {
                                {console.log(currentValue.value, index)}
                                <option value={currentValue.value} key={index}>{currentValue.label}</option>
                              })} */}
                            {/* </Select> */}
                            <Form.Select
                                id="driver"
                    ...
                    <FormGroup className="mb-3">
                        ...
                        {formik.touched.drop_off && formik.errors.drop_off && (
                            <Form.Text className="text-danger">
                                {formik.errors.drop_off}
                            </Form.Text>
                        )}
                    </FormGroup>
                    <FormGroup className="mb-3">
                        <Form.Label htmlFor="pickup_date">
                            Pickup Date
                        </Form.Label>
                        <InputGroup className="mb-3">
                            <InputGroup.Text id="basic-addon1">
                    ...
                    <FormGroup className="mb-3">
                        ...
                        {formik.touched.total_fare &&
                            formik.errors.total_fare && (
                                <Form.Text className="text-danger">
                                    {formik.errors.total_fare}
                                </Form.Text>
                            )}
                    ...
                    <FormGroup className="mb-3">
                        <Form.Label htmlFor="driver_fare">
                            Driver Fare
                        </Form.Label>
                        <InputGroup className="mb-3">
                            <InputGroup.Text id="basic-addon1">
                        ...
                        {formik.touched.driver_fare &&
                            formik.errors.driver_fare && (
                                <Form.Text className="text-danger">
                                    {formik.errors.driver_fare}
                                </Form.Text>
                            )}
